...
Point CatmullCurveEvaluator::calculateCatmull(float t, const Point& p1, const Point& p2, const Point& p3, const Point& p4) const
{
	Point result;
	...
	if (bWrap) {
		for (int i = 0; i < iCtrlPtCount; i++)
		{
			...
			if (i + 1 == iCtrlPtCount) {
				...
				for (int j = 0; j < 100; j++) {
					Point evalPt = calculateCatmull(j * 0.01, ptvCtrlPts[i], ptvEnd1, ptvEnd2, ptvEnd3);
				...
				Point ptvEnd2(ptvCtrlPts[1].x + fAniLength, ptvCtrlPts[1].y);
				for (int j = 0; j < 100; j++) {
					Point evalPt = calculateCatmull(j * 0.01, ptvCtrlPts[i], ptvCtrlPts[i + 1], ptvEnd1, ptvEnd2);
					if (evalPt.x > fAniLength) {
						evalPt.x = evalPt.x - fAniLength;
						prev_eval_x = 0.0;
					}
				...
				Point ptvEnd1(ptvCtrlPts[0].x + fAniLength, ptvCtrlPts[0].y);
				for (int j = 0; j < 100; j++) {
					Point evalPt = calculateCatmull(j * 0.01, ptvCtrlPts[i], ptvCtrlPts[i + 1], ptvCtrlPts[i + 2], ptvEnd1);
					if (evalPt.x > fAniLength) {
						evalPt.x = evalPt.x - fAniLength;
						prev_eval_x = 0.0;
					}
				...
				for (int j = 0; j < 100; j++) {
					Point evalPt = calculateCatmull(j * 0.01, ptvCtrlPts[i], ptvCtrlPts[i + 1], ptvCtrlPts[i + 2], ptvCtrlPts[i + 3]);
					if (evalPt.x > prev_eval_x) {
						ptvEvaluatedCurvePts.push_back(evalPt);
						prev_eval_x = evalPt.x;
					}
